...
export default function S3Uploader() {
  ...
  const validateS3Config = () => {
    const isValid = s3Config.endpoint && s3Config.accessKeyId && s3Config.secretAccessKey && s3Config.bucket;
  ...
  const handleUploadConfigChange = (field: keyof UploadConfig, value: string) => {
  ...
  const testS3Connection = async () => {
    ...
    try {
      ...
      setStatusMessage('✅ Conexión S3 exitosa');
      setUploadStatus('success');
    } catch (error) {
      console.error('Error de conexión S3:', error);
      setStatusMessage(`❌ Error de conexión: ${error instanceof Error ? error.message : 'Error desconocido'}`);
      setUploadStatus('error');
    }
  };
  ...
  const uploadFile = async () => {
    ...
    try {
      // Convertir el archivo a ArrayBuffer para evitar problemas con streams
      ...
      const fileName = uploadConfig.fileName || selectedFile.name;
      const objectKey = uploadConfig.folderName 
        ? `${uploadConfig.folderName}/${fileName}`
        : fileName;
      ...
      setStatusMessage(`✅ Archivo subido exitosamente: ${objectKey}`);
      setUploadStatus('success');
    } catch (error) {
      console.error('Error al subir archivo:', error);
      setStatusMessage(`❌ Error al subir: ${error instanceof Error ? error.message : 'Error desconocido'}`);
      setUploadStatus('error');
    } finally {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              onChange={(e) => handleUploadConfigChange('folderName', e.target.value)}
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => handleUploadConfigChange('fileName', e.target.value)}
        ...
        <div className="space-y-4">
          ...
          {selectedFile && (
            <div className="p-3 bg-gray-50 dark:bg-gray-700 rounded-md">
              ...
              <p className="text-sm text-gray-600 dark:text-gray-300">
                <strong>Ruta final:</strong> {uploadConfig.folderName ? `${uploadConfig.folderName}/` : ''}{uploadConfig.fileName || selectedFile.name}
              </p>
            </div>
          )}
